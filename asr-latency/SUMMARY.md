...
| Model | params | MRT (ms) | RTF | WER | TTFT (ms) | Final Lag (ms) |
|-------|--------|----------|-----|-----|-----------|----------------|
...
- **MRT (ms)** = Median Response Time — median wall-clock to transcribe a clip (offline, per-clip).
  Length-dependent — **RTF** normalizes it out and is the cleaner speed metric; both are shown.
